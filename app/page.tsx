...
const upsertPlayer = async (player: Player, setPlayers: any) => {
  ...
  const { error: deleteError } = await supabase.from("players").delete().eq("ign", cleanIGN);
  if (deleteError) return console.log("Delete error:", deleteError);

  const { error: insertError } = await supabase.from("players").insert([{ ...player, ign: cleanIGN }]);
...
export default function Home() {
  ...
  const handleSearch = async () => {
    ...
    const { data, error } = await supabase.from("players").select("*").ilike("ign", clean).single();
    if (error || !data) return setSearchError("Player not found");
    setSearchError("");
    ...
    <main className="bg-[#0b0b0b] min-h-screen text-white font-sans">
      ...
      <div className="w-full bg-[#0d0d0d] px-4 sm:px-6 lg:px-12 py-4 flex flex-col sm:flex-row items-center justify-between border-b border-[#222] gap-4 sm:gap-0 relative">
        ...
        <div className="flex items-center gap-4 sm:absolute left-1/2 -translate-x-1/2 flex-wrap justify-center">
          ...
          <a href="https://discord.gg/XgKWMDzWbz" target="_blank" rel="noopener noreferrer"
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <input
            ...
            className="flex-1 px-3 py-1 rounded-md bg-[#111] border border-[#222] focus:outline-none focus:border-yellow-500 min-w-[120px]"
          />
      ...
      {/* TABS */}
      <div className="w-full px-3 sm:px-6 lg:px-12 mt-2 border-b border-[#222]">
        <div className="grid grid-cols-2">
          <Link href="/leaderboard" className="text-center py-3 bg-[#111] border border-[#222] border-b-0 border-r-0 rounded-tl-xl font-semibold tracking-wide hover:bg-[#1a1a1a] animate-fadeInUp">Leaderboard</Link>
          <Link href="/" className="text-center py-3 bg-yellow-500 text-black border border-yellow-400 border-b-0 rounded-tr-xl font-bold tracking-wide hover:brightness-110 animate-fadeInUp">Tierlist</Link>
        </div>
      </div>
            ...
            <div key={tier} className="bg-[#111] border border-[#222] rounded-xl p-3 min-h-[480px] flex flex-col">
              ...
              <div className="flex flex-col gap-2 flex-1">
                {[...tierMap[tier].high, ...tierMap[tier].low].map((p) => {
                  ...
                  return (
                    <div key={p.ign} className="group flex h-[46px] bg-[#0f0f0f] border border-[#222] rounded-md hover:bg-[#151515] transition cursor-pointer"
                      ...
                      <div className="flex items-center pr-2">
                        <div className={`flex items-center justify-center h-5 w-[3px] text-[10px] font-bold rounded-full text-white overflow-hidden transition-all duration-200 group-hover:w-10 group-hover:rounded-md ${isAS ? "bg-red-500" : isEU ? "bg-green-500" : "bg-gray-500"}`}>
                          <span className="opacity-0 group-hover:opacity-100 transition">{p.region.toUpperCase()}</span>
                        </div>
                      </div>
                    </div>
                  );
      ...
      {selectedPlayer && (
        <div className="fixed inset-0 backdrop-blur-md bg-black/40 flex items-center justify-center z-50"
          ...
          <div className="bg-[#111] border border-[#222] rounded-2xl shadow-xl px-6 py-5 flex flex-col items-center gap-3 w-[90%] sm:w-[320px] max-w-[400px]"
               onClick={(e) => e.stopPropagation()}>
            <button className="absolute top-3 right-4 text-white text-lg font-bold opacity-70 hover:opacity-100"
                    onClick={() => setSelectedPlayer(null)}>×</button>
            ...
            <div className="w-[90%] sm:w-[220px] h-[320px] flex items-center justify-center">
              <MinecraftSkinViewer ign={selectedPlayer.ign} />
            </div>
            <div className="bg-yellow-400 text-black font-bold px-4 py-1 rounded-md relative overflow-hidden w-full text-center">
